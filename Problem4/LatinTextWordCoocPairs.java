...
public class LatinTextWordCoocPairs {
	
	static HashMap<String,ArrayList<String>> lemmaMapList = new HashMap<String,ArrayList<String>>();

	public static class LatinWCoocPairsMapper extends Mapper<LongWritable, Text, Text, Text> {
		...
		public void map(LongWritable key, Text line, Context context) throws IOException,InterruptedException { 
			try{
				...
				for (int i=0;i<words.length-1;i++){
					...
					if(lemmaMapList.containsKey(words[i].toLowerCase()) && lemmaMapList.containsKey(words[i+1].toLowerCase())){				
						...
						for(String w1_lemma:w1_lemmas){
							for(String w2_lemma:w2_lemmas){
								context.write(new Text(w1_lemma + ","+w2_lemma), location);
							}
						}	
					}
					else if(lemmaMapList.containsKey(words[i].toLowerCase()) && !lemmaMapList.containsKey(words[i+1].toLowerCase())){
						if(words[i+1].isEmpty())
							continue;
						w1_lemmas = lemmaMapList.get(words[i]);
						for(String w1_lemma:w1_lemmas){
							context.write(new Text(w1_lemma+","+words[i+1] ), location);
						}
					}
					else if(!lemmaMapList.containsKey(words[i].toLowerCase()) && lemmaMapList.containsKey(words[i+1].toLowerCase())){
						if(words[i].isEmpty())
						...
						w2_lemmas = lemmaMapList.get(words[i+1]);
						for(String w2_lemma:w2_lemmas){
							context.write(new Text(words[i] + ","+w2_lemma), location);
						}
					}
					else{
						if(words[i].isEmpty() || words[i+1].isEmpty())
							continue;
						context.write(new Text(words[i]+ ","+words[i+1]), location);
					}
				}
			}
			catch (Exception e) { 
	...
	public static class LatinWCoocPairsReducer extends Reducer<Text,Text,Text,Text>{ 
	...
	public static void main(String[] args) throws Exception {
		...
		Job job = Job.getInstance(conf, "Word co-occurence on Classical Latin text using pairs");
		job.setJarByClass(LatinTextWordCoocPairs.class);
		job.setMapperClass(LatinWCoocPairsMapper.class);
		//job.setCombinerClass(LatinWCReducer.class);
		job.setReducerClass(LatinWCoocPairsReducer.class);
